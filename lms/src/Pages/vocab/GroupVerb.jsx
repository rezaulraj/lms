...
const GroupVerb = () => {
  ...
  const [groupVerbs, setGroupVerbs] = useState([]);
  ...
  const fetchWords = async () => {
    try {
      const response = await axiosInstance.get("/groupVerbs/getAll");
      setGroupVerbs(response.data);
    } catch (error) {
  ...
  const groupedData = groupVerbs.reduce((acc, item) => {
