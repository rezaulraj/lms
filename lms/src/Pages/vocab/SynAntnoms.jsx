...
const SynAntnoms = () => {
  ...
  const categorys = [
    {
      idWordBooks: 5,
      word: "Obstinate",
      meaning: "Stubborn, unwilling to change opinion",
      synonyms: "Stubborn, headstrong",
      antonyms: "Flexible, yielding",
      tricks:
        '"An obstinate person is like an obstacle—they won’t move easily!"',
      sentence:
        '"An obstinate person is like an obstacle—they won’t move easily!"',
      idVocabs: 2,
      mcqQuestion: 'What is the meaning of "Obstinate"?',
      mcqOptions: [
        "Easily persuaded",
      ...
      mcqAnswer: 1,
      mcqTricks: "An obstinate person is like",
      matchingOptions: ["Hardworking", "dedicated", "persistent", "persistent"],
      matchingMeaningOptions: [
        "Stubborn and unyielding",
        "Flexible in thoughts",
        "Easily persuaded",
        "Easily persuaded",
      ],
      matchingAnswerOptions: [0, 1, 2, 3],
      Vocab: {
        idVocabs: 2,
  ...
  const getAllCategory = async () => {
    try {
      const response = await axiosInstance.get("/wordBook/getAll");
  ...
  // Group the data by vocabType and title
  const groupedData = categorys
    .filter((item) => item?.Vocab?.Category?.name === "synonyms-and-antonyms")
    .reduce((acc, item) => {
